<?php
/*
	Les fonctions présentes sur cette page
...
function get_num_licence() {
	global $bdd;
	
	$v = $bdd->query('	SELECT val_option
						FROM aeroport_options
						WHERE nom_option = "num_licence"
					');
...
function get_services($id) {
	global $bdd;
	$v = $bdd->query('	SELECT s.*
						FROM outlet_outlet o JOIN outlet_proposition_services p ON o.id_outlet = p.id_outlet
						JOIN outlet_services s ON p.id_service = s.id_service
						WHERE p.id_outlet = "'.$id.'"
...
function get_outlets_par_service($id_service) {
	global $bdd;
	
	$v = $bdd->query('	SELECT *,s.supplement
						FROM outlet_outlet o JOIN outlet_proposition_services p ON p.id_outlet = o.id_outlet
						JOIN outlet_services s ON p.id_service = s.id_service
						WHERE p.id_service = '.$id_service.'
						ORDER BY o.id_outlet
					');
...
function get_trajet_par_nom_et_date($nom,$date){
	global $bdd;
	...
	$v = $bdd->query('	SELECT *,
								...
								DATE_FORMAT(date_trajet, "%c") as num_mois_trajet
						FROM europa_trajet
						WHERE service_trajet = "'.$nom.'"
						AND DATE(date_trajet) = "'.$date.'"
...
function get_nom_lieu($id){
	...
	$v = $bdd->query('	SELECT nom_lieu_'.$_SESSION['lang'].'
						FROM europa_lieu
						WHERE id_lieu = "'.$id.'"
					');
	...
	$value = $r['nom_lieu_'.$_SESSION['lang']];
...
function est_admin($mail_client) {
	global $bdd;
	
	$v = $bdd->query('	SELECT est_admin
...
function get_nb_passagers($id) {
	...
	$v = $bdd->query('SELECT SUM(r.nb_pers) AS nb_passagers
...
function set_reservation_attente($id){
	global $bdd;
	
	$bdd->exec("	UPDATE europa_reservation
					SET mode_paiement = 'ATTENTE'
					WHERE id_reservation = '".$id."'");
}

function set_reservation_gratuite($id){
	global $bdd;
	
	$bdd->exec("	UPDATE europa_reservation
					SET mode_paiement = 'GRATUIT'
					WHERE id_reservation = '".$id."'");
}
