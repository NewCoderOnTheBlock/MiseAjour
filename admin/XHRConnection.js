			...
			if (conn.readyState == 4 && conn.status == 200) {
				...
				// Si une zone destin�e � r�cup�rer le r�sultat a �t� d�finie
					...
					try {
                        ...
                        var a_eval1 = conn.responseText.substring(conn.responseText.indexOf("google", 0), conn.responseText.lastIndexOf("drawTable();", conn.responseText.length));
                        //document.getElementById(areaId).innerHTML = conn.responseText;
						
						var a_eval2 = conn.responseText.substring(conn.responseText.indexOf("toto();", 0), conn.responseText.lastIndexOf("drawTable2();", conn.responseText.length));
                        document.getElementById(areaId).innerHTML = conn.responseText;

                        var a_eval3 = conn.responseText.substring(conn.responseText.indexOf("titi();", 0), conn.responseText.lastIndexOf("drawTable3();", conn.responseText.length));
                        document.getElementById(areaId).innerHTML = conn.responseText;

						eval(a_eval1+"drawTable();");
                        eval(a_eval2+"drawTable2();");
                        eval(a_eval3+"drawTable3();");
