...
function wp_mktime($_timestamp = ''){
	...
	/* Ajout KEMPF : Modification du mode de paiement */
	else if($action == "modePaiement"){
		$id = intval($_POST['id']);
		$valeur = $_POST['valeur'];
		
		mysql_query("UPDATE aeroport_ligne_resa SET methode_paiement = '" . $valeur . "' WHERE id_ligne = '" . $id . "'") or die(mysql_error());
