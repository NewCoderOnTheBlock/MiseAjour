...
while ($r3 = @mysql_fetch_assoc($result3)){
	...
	$detail.="- ".$civilite." ".stripslashes($nom)." ".stripslashes($prenom)." (".$nb_pers." adulte(s) + ".$nb_enfant." enfant(s) ), ".stripslashes($rassemblement).", ".$heure."
...
$content_mail = "Bonjour " . $r21['prenom'] . ", <br /><br />Un petit mail pour vous informer que sur votre navette de " . $row_depart['nom'] . " vers " . $row_retour['nom'] . " du " . $row_trajet['date'] . ", une personne c'est rajoutée.<br /><br />Vous pouvez consulter l'agenda pour vérifier. <br /><br />Bonne route !";
    ...
    if($mode == "online")
    {
        mail($r21['mail'], "Une personne en plus | " . utf8_decode($row_trajet['date']), $content_mail, header_mail());
    }
    else
    {
        file_put_contents("unepersonneneplus.html", $content_mail.$r21['mail']);
    }
