...
$req = "DELETE FROM aeroport_ligne_resa WHERE id_ligne = ".$ligneResa;
mysql_query($req)or die(mysql_error());

$reqPaye = mysql_query("SELECT est_paye FROM aeroport_ligne_resa WHERE id_trajet = '" . $numTrajet . "'") or die(mysql_error());
$ok = true;
...
$libelle = $nom_depart." - ".$nom_dest . " (" . $nb_pers_total . " personnes )";
    ...
    {
        file_put_contents("fd2.html", $content_mail.$row_chauff['mail']);
    }
	...
	if($numLigne == 0){
		...
		if($estValid ==1){
            ...
            {
                file_put_contents("fd.html", $content_mail.$row_chauff['mail']);
            }
