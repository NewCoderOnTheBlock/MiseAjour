...
function Session_ok($idSession, $fromPPX = false) {
  ...
  $DB_CX->DbQuery("SELECT util_id, util_interface, util_telephone_vf, util_planning, sid_util_subst_id, sid_semaine_type, sid_filtre_couleur, sid_screen, admin_id, util_format_nom, util_menu_dispo, util_note_barree, util_menuonclick, util_langue, CONCAT(util_nom,' ',util_prenom) AS nomUtil, util_fcke, util_fcke_toolbar, droit_profils, droit_agendas, droit_notes, droit_admin FROM ${PREFIX_TABLE}sid LEFT JOIN ${PREFIX_TABLE}admin ON admin_id=sid_admin_id, ${PREFIX_TABLE}utilisateur, ${PREFIX_TABLE}droit WHERE sid_id='".$idSession."' AND sid_session_id='".@session_id()."' AND sid_util_id = util_id AND droit_util_id=util_id");
