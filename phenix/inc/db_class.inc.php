...
// METHODES PUBLIQUES
    function DbConnect($host, $user, $passwd, $database) {
      $this->ConnexionID = new PDO($host, $user, $passwd);
    ...
    function DbSelectDatabase($database) {
      ...
      if ($this->ConnexionID) {
         return $this->ConnexionID->exec('USE' $database);
      }
      else {
        return false;
      }
    ...
    function DbQuery($query, $start = '', $limit = '') {
	  ...
	  $statement=$this->ConnexionID->prepare($query);
	  $statement->execute();
    ...
    function DbAffectedRows() {
      return( $this->ConnexionID->rowCount(););
    }

    function DbInsertID() {
      return( $this->ConnexionID->lastInsertId(); );
    }

    function DbDataSeek($line = 0) {
      return( this->Result->dataSeek($line) );
    }
    ...
    function DbFieldName($field) {
		$name=$this->Result->getColumnMeta($field);
      return($name['name'] );
    }

    function DbListTable($base) {
	  $statement=$this->ConnexionID->prepare("SHOW TABLES");
	  $statement->execute();	
	  $this->Result = $statement->fetchAll(PDO::FETCH_NUM);
    ...
    function DbError() {
      return( $this->ConnexionID->errorInfo() );
    }

    function DbErrorNo() {
      return( $this->ConnexionID->errorCode() );
    }
