<?php
	...
	/* 
		Calcul du prix
	*/
	$prix = get_value_of_option("tarif_min_aller_retour");
	// Majoration si jours différents
	if ($_POST['jour_aller'] != $_POST['jour_retour'])
	{
		$prix += get_value_of_option("supplement_differe");
	}
	...
	// Majoration si horaire à la demande
	if (($_POST['radio_heure_aller'] == "demande" || $_POST['radio_heure_retour'] == "demande"))
	{
		$prix += get_value_of_option("maj_demande");
		$_SESSION["trajet"]["demande"] = true;
	}
	else
	{
		$_SESSION["trajet"]["demande"] = false;
	}
	...
	$_SESSION["trajet"]["jour_aller_long"] = $_POST['jour_aller_long'];
	$_SESSION["trajet"]["jour_retour_long"] = $_POST['jour_retour_long'];
	$_SESSION["trajet"]["jour_aller"] = $_POST['jour_aller'];
	$_SESSION["trajet"]["jour_retour"] = $_POST['jour_retour'];
	...
	$_SESSION["trajet"]["adresse_retour"] = ($_POST['select_lieu_retour'] == 4) ? $_POST['txt_adresse_compl_retour']."<br />".$_POST['txt_cp_compl_retour']."<br />".$_POST['txt_ville_compl_retour'] : "";
	
	// Date Aller
	$heure_aller = 0;
	$minute_aller = 0;
	if (($_POST['radio_heure_aller'] == "demande")){
		$heure_aller = $_POST['select_heure_aller_demande'];
		$minute_aller = $_POST['select_minute_aller_demande'];
	}else{
		list($heure_aller, $minute_aller) = split(':', $_POST['select_heure_aller_fixe']);
	}
	
	// On formate la date aller
	$d_explode = explode('-',$_POST['jour_aller']);
	...
	$heure_retour = 0;
	$minute_retour = 0;
	if (($_POST['radio_heure_retour'] == "demande")){
		$heure_retour = $_POST['select_heure_retour_demande'];
		$minute_retour = $_POST['select_minute_retour_demande'];
	}else{
		list($heure_retour, $minute_retour) = split(':', $_POST['select_heure_retour_fixe']);
	}
	
	// On formate la date retour
	$d_explode = explode('-',$_POST['jour_retour']);
