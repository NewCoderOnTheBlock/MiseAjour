<?php
	...
	*/
	...
	if (isset($fac_id_facture)){
		...
		//Info Client
		$fac_client_c = $fac_res['adresse_facture'];
		$fac_adresse = array();
		$fac_adresse = explode("|", $fac_client_c);
		if (empty($fac_client_c))
		{
		...
		if($fac_adresse[0] == '1' )
		{
			$fac_civilite_c = $fac_adresse[1];
			$fac_nom_c = $fac_adresse[2];
			$fac_prenom_c = $fac_adresse[3];
			$fac_mail_c = $fac_res["email"];
		}
		else if($fac_adresse[0] == '2')
		{
			$fac_civilite_c = $fac_res["civilite"];
			$fac_nom_c = $fac_res["nom"];
			$fac_prenom_c = $fac_res["prenom"];
			$fac_adresse_postale_c = $fac_adresse[1];
			$fac_adresse_ville_c = $fac_adresse[2];
			$fac_code_postal_c = $fac_adresse[3];
			$fac_pays_c = $fac_adresse[4];
		}
		else if($fac_adresse[0] == '3')
		{
			$fac_civilite_c = $fac_adresse[1];
			$fac_nom_c = $fac_adresse[2];
			$fac_prenom_c = $fac_adresse[3];
			$fac_adresse_postale_c = $fac_adresse[4];
			$fac_adresse_ville_c = $fac_adresse[5];
			$fac_code_postal_c = $fac_adresse[6];
			$fac_pays_c = $fac_adresse[7];
		}
		...
		include("facture_aeroport_2.php");
