/*
	KEMPF
	Ajout des champs d'autocomplétion
	afin de permettre une saisie plus rapide et
	plus intuitive de la réservation
*/

var langue = document.getElementById("page_lang").value;
if (langue == "fr"){
	var array_prov_dest = [	"Rome", 
							"Hambourg", 
							"Bari", 
							"Londres", 
							"Porto", 
...
$(document).ready(function() {
	...
	$("input#compagnie_depart_vol").autocomplete({
		source: ["Ryan Air", "Air Berlin", "Air France", "EasyJet", "SunExpress", "Lufthansa", "Pegasus Airlines", "Turkish Airlines", "Austrian Airlines", "Ukrainian Airline", "Aerlingus", "Alitalia"],
		minLength: 2
	});
	$("input#compagnie_retour_vol").autocomplete({
		source: ["Ryan Air", "Air Berlin", "Air France", "EasyJet", "SunExpress", "Lufthansa", "Pegasus Airlines", "Turkish Airlines", "Austrian Airlines", "Ukrainian Airline", "Aerlingus", "Alitalia"],
		minLength: 2
	});
	// Provenances / Destinations
	$("input#provenance_depart_vol_1").autocomplete({
		source: array_prov_dest,
		minLength: 2
	});
	$("input#provenance_depart_vol_2").autocomplete({
		source: array_prov_dest,
		minLength: 2
	});
	$("input#provenance_retour_vol_1").autocomplete({
		source: array_prov_dest,
		minLength: 2
	});
	$("input#provenance_retour_vol_2").autocomplete({
		source: array_prov_dest,
		minLength: 2
	});
});
...
var horaire_choisi_aller = document.querySelector('#horaire_aller > input');
var horaire_choisi_retour = document.querySelector('#horaire_retour > input');

/* 
	KEMPF : 
	Elements de texte qui change
	en fonction de la demande.
	Si on part de Strasbourg, le premier champs de l'heure
	du vol sera "Départ du vol"
*/
var elem_text_heure_vol_1 = document.getElementById("text_heure_vol_1");
var elem_text_heure_vol_2 = document.getElementById("text_heure_vol_2");
var elem_text_rassemblement_aller = document.getElementById("label_pt_rassemblement_aller");
var elem_text_rassemblement_retour = document.getElementById("label_pt_rassemblement_retour");

if (langue == "fr"){
	var text_arrivee_vol = "Heure d'arrivée du vol";
	var text_depart_vol = "Heure de départ du vol";
	var text_prise_strasbourg = "Point de prise sur Strasbourg";
	var text_depose_strasbourg = "Point de dépose sur Strasbourg";
}else{
	var text_arrivee_vol = "Arrival time of flight";
	var text_depart_vol = "Departure time of flight";
	var text_prise_strasbourg = "Pick-up in Strasbourg";
	var text_depose_strasbourg = "Drop-off in Strasbourg";
}
var liste_depart = document.getElementById('lst_trajet_depart');
if(liste_depart.options[liste_depart.selectedIndex].value != 100){
...
if(lst_passager_enfant_aller && lst_passager_enfant_aller.selectedIndex != 0)
	div_bebe_aller.style.display = "block";
	
/*if(lst_passager_enfant_retour && lst_passager_enfant_retour.selectedIndex != 0)
...
if(trajet_aller_retour.checked)
{
	...
	var div_rass_retour = document.getElementById('div_rass_retour');
	if (div_rass_retour)
	{
		div_rass_retour.style.display = "none";
	}
...
// KEMPF : Suppression de la limitation des horaires fixes pour certains aéroports
/*if(
	// Bale Mulhouse
	...
	lst_trajet_depart.options[lst_trajet_depart.selectedIndex].value == 10 ||
	lst_trajet_arrive.options[lst_trajet_arrive.selectedIndex].value == 10 
)
{*/
	if(!trajet_aller_retour.checked)
	{
		...
		sendRequest('lst_fixe_depart', "depart", type, heure_fixe_aller.value, id_lieu, date_demande);
	...
	else
	{
		...
		sendRequest('lst_fixe_retour', "retour", type, heure_fixe_retour.value, id_lieu, date_demande);
...
var btn_envoie = document.getElementById("btn_envoie_res");
...
function remove_image()
{
	...
	try {
		...
		for(var i = pic.length; i > 0; i--)
			if(pic[i-1].className != "pointer" && pic[i-1].className != "no-class")
...
function verif()
{
	...
	if(lst_heure_depart.selectedIndex == 0 && document.getElementById('lst_fixe_depart').selectedIndex == 0 && (horaire_choisi_aller == null || horaire_choisi_aller.value == ''))
	{
		res = false;
	...
	if(trajet_aller_retour.checked)
	{
		if(lst_heure_retour.selectedIndex == 0 && document.getElementById('lst_fixe_retour').selectedIndex == 0 && (horaire_choisi_retour == null || horaire_choisi_retour.value == ''))
		{
			res = false;
...
//lst_trajet_arrive.onchange = function (){horaire_fixe_retour();gestion_prov_dest();}

lst_fixe_depart.onchange = function() { exclusion("lst_heure_depart"); var div_aller = document.getElementById('horaire_aller'); if(div_aller) { div_aller.style.display = 'none';div_aller.querySelector('input').value = '';}}
lst_fixe_retour.onchange = function() { exclusion("lst_heure_retour"); var div_retour = document.getElementById('horaire_retour'); if(div_retour) { div_retour.style.display = 'none';div_retour.querySelector('input').value = '';}}

lst_heure_depart.onchange = function() { exclusion("lst_fixe_depart"); var div_aller = document.getElementById('horaire_aller'); if(div_aller) { div_aller.style.display = 'none';div_aller.querySelector('input').value = '';}}
lst_heure_retour.onchange = function() { exclusion("lst_fixe_retour"); var div_retour = document.getElementById('horaire_retour'); if(div_retour) { div_retour.style.display = 'none';div_retour.querySelector('input').value = '';}}
