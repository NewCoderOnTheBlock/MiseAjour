<?php
	...
	if(count($_GET) == 8)
	{
		...
		// Ajout KEMPF : ID paypal
		$id_paypal = getIdPaypal($id_res);
		...
		else
		{
			...
			else
			{
				...
				$sql_res = query("	SELECT 	id_client, 
											DATE_FORMAT(date, '%d-%m-%Y') AS date_jour, 
											DATE_FORMAT(date, '%H') AS date_heure, 
											DATE_FORMAT(date, '%i') AS date_min, 
											DATE_FORMAT(date, '%s') AS date_sec, 
											commentaire, estSimple, 
											supplement, 
											res_der_min,
											opt_annulation,
				...
				else
				{
					...
					else
					{
						...
						$sql_trajet1 = query("SELECT id_lieu_depart, id_lieu_dest, DATE_FORMAT(date, '%d-%m-%Y') AS date_jour, DATE_FORMAT(date, '%Y-%m-%d %H:%i:%s') AS date_en, DATE_FORMAT(date, '%Hh%i') AS date_heure, estFixe, est_paye
						...
						{
							$sql_ligne_resa1 = query("SELECT id_res, id_trajet, id_pt_rass, rassemblement, info_vol, nb_pers, nb_enfant, heure, prix, estFixe, supplement, est_nuit, prix_base, rajout, est_paye, type_trajet, DATE_FORMAT(heure, '%H:%i') AS heure_aller
							...
							else
							{
								...
								else
								{
									...
									$date_aller = $row_trajet1['date_en'];
									...
									$tpl->set(array(
													...
													"INFO_VOL_ALLER" => nl2br($row_ligne1['info_vol']),
													...
													"TXT_SURCOUT_ALLER" => ($row_ligne1['estFixe'] == "0") ? 15 : 0,
									...
									{
										$sql_trajet2 = query("SELECT id_lieu_depart, id_lieu_dest, DATE_FORMAT(date, '%d-%m-%Y') AS date_jour, DATE_FORMAT(date, '%Y-%m-%d %H:%i:%s') AS date_en, DATE_FORMAT(date, '%Hh%i') AS date_heure, estFixe, est_paye
															FROM aeroport_trajet
										...
										{
											$sql_ligne_resa2 = query("SELECT id_res, id_trajet, id_pt_rass, rassemblement, info_vol, nb_pers, nb_enfant, heure, prix, estFixe, supplement, est_nuit, prix_base, rajout, est_paye, DATE_FORMAT(heure, '%H:%i') AS heure_retour
																	 FROM aeroport_ligne_resa
											...
											else
											{
												...
												$date_retour = $row_trajet2['date_en'];
												...
												$tpl->set(array(
																...
																"INFO_VOL_RETOUR" => nl2br($row_ligne2['info_vol']),
																...
																"TXT_SURCOUT_RETOUR" => ($row_ligne2['estFixe'] == "0") ? 15 : 0,
		...
		else
		{
			...
			$majoration_option_annulation = 0;
			$opt_annulation = 0;
			...
			if($row_ligne1['type_trajet'] == 'ALLER')
			{
				...
				if ($row_res['opt_annulation'] == 1){
					
					$opt_annulation = 1;
					$majoration_option_annulation = $row_res['montant_opt_annulation'];
					$prix_total_a_payer += $majoration_option_annulation;
			...
			if($row_res['estSimple'] == "0" && $ar == "1")
			{
				...
				if($row_ligne2['estFixe'] == "0")
					$prix_total_a_payer += get_option("maj_surcout_demande");
			}
			
			// KEMPF : Gestion des horaires de nuit
			$montant_maj_nuit = get_option("maj_horaire_nuit");
			$majoration_nuit_aller = ($row_ligne1['est_nuit'] == 1) ? $montant_maj_nuit : 0;
			if($row_res['estSimple'] == "0" && $ar == "1")
			{
				$majoration_nuit_retour = ($row_ligne2['heure_retour'] == 1) ? $montant_maj_nuit : 0;
			}
			$prix_total_a_payer += $majoration_nuit_aller + $majoration_nuit_retour;
			
			// KEMPF : Majoration jour ferié ALLER OUI/NON
			$resultat_majoration_jour_aller = get_jour_majore($date_aller);
			...
			$resultat_majoration_jour_retour = get_jour_majore($date_retour);
			...
			$prix_total_a_payer += $majoration_jour_special_aller + $majoration_jour_special_retour;
						
						
			$lang_paypal = (strtolower($_SESSION['lang']) == 'fr') ? 'FR' : 'GB';
			$custom = $id_paypal."|0|0|" . $_SESSION['lang'] . "|0|1|" . $id_ligne1;
			...
			$tpl->set(array(
							...
							"MONTANT_OPTION_ANNULATION" => $majoration_option_annulation,
