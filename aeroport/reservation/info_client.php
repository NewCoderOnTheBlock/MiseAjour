<?php
	...
	/*
		...
		Cette page sert principalemeent à remplir la variable de 
		session après que le client ai effectué une reservation.
		
	*/
	
	unset($_SESSION['fin_resa']);
	$_SESSION['fin_demande'] = true;
	if(!isset($_POST['lst_passager_adulte_retour'])){
		$_POST['lst_passager_adulte_retour']="";
		$_POST['lst_passager_enfant_retour']="";
		$_POST['lst_passager_enfant_retour_g0']="";
		$_POST['lst_passager_enfant_retour_g1']="";
		$_POST['lst_passager_enfant_retour_g2']="";
		$_POST['lst_passager_enfant_retour_g3']="";
		$_POST['nouvel_horaire_retour']="";
		$_POST['code_promo']="";
	}
	if(!isset($_SESSION['client']['adresse'])){
		$_SESSION['client']['ind_fixe']="";
		$_SESSION['client']['ind_port']="";
		$_SESSION['client']['cp']="";
		$_SESSION['client']['ville']="";
	...
	if(isset($_POST['res_1']) || isset($_GET['res_1']))
	{
		...
		if(!isset($_SESSION['info_trajet_ok']))
		{
			...
			$_SESSION['trajet']['opt_annulation'] = (isset($_POST['opt_annulation'])) ? 1 : 0;
			...
			if(!isset($_POST['nouvel_horraire_aller'])){
				$_POST['nouvel_horaire_aller'] = '';
			}
			
				if($_POST['nouvel_horaire_aller'] == '')
				{
			...
			if($_SESSION['trajet']['type_trajet'] == 0)
			{
				/*if(in_array($_SESSION['trajet']['depart'], $tab_id_fixe) || in_array($_SESSION['trajet']['dest'], $tab_id_fixe))
				{*/
					if($_POST['nouvel_horaire_retour'] == '')
					{
		...
		/*
			KEMPF :
			Ajout de la particularité d'Entzheim (Qui n'a pas d'horaire à la demande)
		*/
		...
		else
		{
			if($_SESSION['trajet']['type_trajet'] == 0 && (strlen($_SESSION['trajet']['date_retour']) <= 0 || strlen($_SESSION['trajet']['date_retour_long']) <= 0 || strlen($_SESSION['trajet']['heure_retour']) <= 0 || $_SESSION['trajet']['pt_rass_aller'] <= 0 || strlen($_SESSION['trajet']['compagnie_retour_vol']) <= 0 || strlen($_SESSION['trajet']['provenance_retour_vol']) <= 0 || $_SESSION['trajet']['heure_retour_vol'] == "-" || $_SESSION['trajet']['minute_retour_vol'] == "-"))
			{
				$ok = false;
				$_SESSION['erreur_erreur_ok'] = false;
				$_SESSION['erreur_erreur'] = $erreur_champ_vide2;
			}
			else
			{
				if($_SESSION['trajet']['type_trajet'] == 0)
